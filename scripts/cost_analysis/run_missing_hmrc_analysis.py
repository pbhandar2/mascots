import argparse, itertools, time, json, pathlib 
import pandas as pd 
import numpy as np 
...
def get_missing_cost_array(workload_name, num_points, step_size, mt_label="D1_D3_H3"):
    ...
    evaluated_ex_cost_array = np.array([], dtype=int)
    if ex_out_path.exists():
        ex_df = pd.read_csv(ex_out_path, names=OPT_ROW_JSON)
        evaluated_ex_cost_array = ex_df["cost"].to_numpy()
    ...
    remaining_cost_array = np.setdiff1d(evaluated_ex_cost_array, evaluated_hmrc_cost_array)

    if len(remaining_cost_array) > 0:
        indexes = np.linspace(0, len(remaining_cost_array)-1, num_points, dtype=int)
        return list(set(remaining_cost_array[indexes]))
    else:
        return []
